...
const drawGrid = () => {
  ...
  const fraction = 90/(1.5*numEntries);
  while (gridContainer.hasChildNodes()){
    gridContainer.childNodes.forEach( x => 
      gridContainer.removeChild(x) 
    )
  }
  ...
  document.querySelectorAll('.col').forEach( box => {
    box.style.width = String(fraction).concat('vmin');
    box.style.height= String(fraction).concat('vmin');
  });
